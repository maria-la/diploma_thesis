...
/* Private functions */
static void Householders_Reduction_to_Bidiagonal_Form(float32_t *A, uint16_t nrows, uint16_t ncols, float32_t *U, float32_t *V, float32_t *diagonal, float32_t *superdiagonal);
static int Givens_Reduction_to_Diagonal_Form(uint16_t nrows, uint16_t ncols, float32_t *U, float32_t *V, float32_t *diagonal, float32_t *superdiagonal);
static void Sort_by_Decreasing_Singular_Values(uint16_t nrows, uint16_t ncols, float32_t *singular_value, float32_t *U, float32_t *V);
 ...
 * Return 0 = Fail.
 */
uint8_t svd_golub_reinsch(float32_t A[], uint16_t row, uint16_t column, float32_t U[], float32_t S[], float32_t V[]) {
	float32_t dummy_array[column];
...
static void Householders_Reduction_to_Bidiagonal_Form(float32_t *A, uint16_t nrows, uint16_t ncols, float32_t *U, float32_t *V, float32_t *diagonal, float32_t *superdiagonal) {
	int i, j, k, ip1;
	float32_t s, s2, si, scale;
	float32_t *pu, *pui, *pv, *pvi;
	float32_t half_norm_squared;

	/* Copy A to U */
	memcpy(U, A, sizeof(float32_t) * nrows * ncols);
...
static void Sort_by_Decreasing_Singular_Values(uint16_t nrows, uint16_t ncols, float32_t *singular_values, float32_t *U, float32_t *V) {
	int i, j, max_index;
	float32_t temp;
	float32_t *p1, *p2;
